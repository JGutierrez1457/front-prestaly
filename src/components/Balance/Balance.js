...
function Balance({ creator, file, balance, date}) {
    ...
    return (
        ...
        {balance.map( (b, index)=>(<div className={classes.balance} key={index}><b>{b.username}:&nbsp;</b><b style={{ color : b.amount<0?'#f44336':'#3f51b5', }}>{((b.amount<0)?('-s/'):('s/'))+Math.abs(b.amount)}</b></div>))}
        </div>
        </div>
    )
}
